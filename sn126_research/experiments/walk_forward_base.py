"""Walk-forward: base-30 vs 35-clean (do the 5 additions generalize out-of-time?).

Same protocol as walk_forward.py: expanding window, train on earlier dates,
validate on later UNSEEN dates. Model FIXED. No new features.

35-clean = base-30 + betsize_street_std_mean + action_bigram_entropy
           + pot_escalation_mean + mean_aggressors_per_hand + potodds_call_fold_gap

Run: python -m sn126_research.experiments.walk_forward_base
"""
...
def main() -> None:
    ...
    add5 = [idx["p2:betsize_street_std_mean"], idx["p2:action_bigram_entropy"],
    ...
    per_fold = {"30": [], "35": []}
    pooled = {"30": ([], []), "35": ([], [])}
    hdr = f"{'val_date':12s} {'ntr':>4s} {'nva':>4s} | {'AP30':>6s} {'rec30':>6s} {'cal30':>6s} | {'AP35':>6s} {'rec35':>6s} {'cal35':>6s} | {'dRec':>6s} {'dCal':>6s}"
    print(hdr); print("-" * len(hdr))
    for k in range(2, len(uniq)):
        vd = uniq[k]
        tr = np.array([d in uniq[:k] for d in dates]); va = dates == vd
        if len(np.unique(y[tr])) < 2 or len(np.unique(y[va])) < 2:
            continue
        row = {}
        for tag, cols in (("30", []), ("35", add5)):
            m = gbm().fit(X(cols)[tr], y[tr])
            p = m.predict_proba(X(cols)[va])[:, 1]
            ev = evaluate(y[va], p)
        ...
        a0, r0, c0 = row["30"]; a5, r5, c5 = row["35"]
        print(f"{vd:12s} {int(tr.sum()):4d} {int(va.sum()):4d} | {a0:6.3f} {r0:6.3f} {c0:6.3f} | "
              f"{a5:6.3f} {r5:6.3f} {c5:6.3f} | {r5-r0:+6.3f} {c5-c0:+6.3f}")

    print("\n=== per-fold mean +/- std (out-of-time) ===")
    for tag in ("30", "35"):
        arr = np.array(per_fold[tag]); mu, sd = arr.mean(0), arr.std(0)
        print(f"  set-{tag}: AP={mu[0]:.4f}+/-{sd[0]:.3f}  recall={mu[1]:.4f}+/-{sd[1]:.3f}  cal_reward={mu[2]:.4f}+/-{sd[2]:.3f}")

    print("\n=== POOLED out-of-time ===")
    for tag in ("30", "35"):
        s = np.array(pooled[tag][0]); yy = np.array(pooled[tag][1]); ev = evaluate(yy, s)
        print(f"  set-{tag}: AP={ev['ap']:.4f}  recall@5%={ev['recall_at_5pct_fpr']:.4f}  "
              f"cal_reward={cal(ev['ap'], ev['recall_at_5pct_fpr']):.4f}  (n={len(yy)})")

    d_cal = [f5[2] - f0[2] for f0, f5 in zip(per_fold["30"], per_fold["35"])]
    d_rec = [f5[1] - f0[1] for f0, f5 in zip(per_fold["30"], per_fold["35"])]
    pos = sum(1 for x in d_cal if x > 0)
    print("\n=== 5 additions: out-of-time consistency (35 - 30) ===")
    print(f"  per-fold ΔcalReward: {[round(x,3) for x in d_cal]}")
    print(f"  per-fold Δrecall   : {[round(x,3) for x in d_rec]}")
    print(f"  folds improved: {pos}/{len(d_cal)} | mean ΔcalReward={np.mean(d_cal):+.4f} "
          f"(std {np.std(d_cal):.4f}) | mean Δrecall={np.mean(d_rec):+.4f}")
    verdict = ("CONSISTENT -> the 5 additions generalize; ship 35-clean"
               if pos >= max(2, int(0.7 * len(d_cal))) and np.mean(d_cal) > 0
               else "INCONSISTENT -> additions may be noise; consider base-30")
